...
src_compile() {
	# Building bundled LLVM requires ~2GB per job
	local max_jobs=$(( $(free -m | awk '/Mem:/ {print $7}') / 2048 ))
	max_jobs=$(( max_jobs < 1 ? 1 : max_jobs ))

	einfo "Limiting to ${max_jobs} parallel jobs to avoid OOM"
	MAKEOPTS="-j${max_jobs}" cmake_src_compile
}
