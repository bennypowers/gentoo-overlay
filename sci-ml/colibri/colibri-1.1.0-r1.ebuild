...
src_prepare() {
	default
	# Upstream's coli adds its own directory to sys.path for the version
	# import. When installed to /usr/bin, version.py lives in libexec
	# instead. Insert libexec into sys.path before the import.
	sed -i '/^sys\.path\.insert(0, HERE)/a sys.path.insert(0, os.path.join(os.path.dirname(HERE), "libexec", "colibri"))' \
		c/coli || die
}
...
src_install() {
	...
	if use python; then
		...
		doins c/openai_server.py c/resource_plan.py c/doctor.py c/version.py
	fi
