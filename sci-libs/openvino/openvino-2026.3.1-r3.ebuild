...
SRC_URI="
	...
	js? (
		https://github.com/bennypowers/gentoo-overlay/releases/download/openvino-node-api-headers-v1.9.0/node-api-headers-v1.9.0.tar.gz
			-> ${P}-node-api-headers.tar.gz
		https://github.com/bennypowers/gentoo-overlay/releases/download/openvino-node-addon-api-v8.9.0/node-addon-api-v8.9.0.tar.gz
			-> ${P}-node-addon-api.tar.gz
	)
"
...
PATCHES=(
	...
	"${FILESDIR}/${P}-node-deps.patch"
	"${FILESDIR}/${P}-gpu-lib64.patch"
)
...
src_unpack() {
	...
	if use js; then
		_unpack_submodule "${P}-node-api-headers.tar.gz" "src/bindings/js/node/thirdparty/node-api-headers"
		_unpack_submodule "${P}-node-addon-api.tar.gz" "src/bindings/js/node/thirdparty/node-addon-api"
	fi
}
