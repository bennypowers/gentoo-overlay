...
RDEPEND="
	...
	$(python_gen_cond_dep '
		...
		~dev-python/comfy-kitchen-0.2.20[${PYTHON_USEDEP}]
		...
		~dev-python/comfyui-frontend-package-1.45.21[${PYTHON_USEDEP}]
		~dev-python/comfyui-workflow-templates-0.11.12[${PYTHON_USEDEP}]
		~dev-python/comfyui-embedded-docs-0.5.8[${PYTHON_USEDEP}]
	')
	media-libs/mesa[egl(+)]
	...
	cuda? ( $(python_gen_cond_dep '~dev-python/comfy-aimdo-0.4.10[${PYTHON_USEDEP}]') )
	migraphx? (
...
src_install() {
	...
	# comfy_angle bundles ANGLE EGL/GLESv2 from Electron.
	# Install a stub that uses system mesa instead.
	insinto "${instdir}/comfy_angle"
	doins "${FILESDIR}"/comfy_angle_stub/*.py
