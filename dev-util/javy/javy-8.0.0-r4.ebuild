...
pkg_setup() {
	...
	# Verify wasm32-wasip1 target is available. Gentoo's rust_sysroots_wasm
	# only provides wasm32-unknown-unknown; wasip1 must be added manually.
	local sysroot
	sysroot="$(rustc --print sysroot)" || die
	if [[ ! -d "${sysroot}/lib/rustlib/wasm32-wasip1" ]]; then
		eerror "The wasm32-wasip1 Rust target is not installed."
		eerror "Gentoo's rust_sysroots_wasm USE flag only provides wasm32-unknown-unknown."
		eerror ""
		eerror "Install the target manually:"
		eerror "  rustup target add wasm32-wasip1"
		eerror ""
		eerror "Or download and install it from the Rust release:"
		eerror "  RUST_VER=\$(rustc --version | cut -d' ' -f2)"
		eerror "  wget https://static.rust-lang.org/dist/rust-std-\${RUST_VER}-wasm32-wasip1.tar.xz"
		eerror "  tar xf rust-std-\${RUST_VER}-wasm32-wasip1.tar.xz"
		eerror "  cp -a rust-std-\${RUST_VER}-wasm32-wasip1/rust-std-wasm32-wasip1/lib/rustlib/wasm32-wasip1 \\"
		eerror "    \$(rustc --print sysroot)/lib/rustlib/"
		die "wasm32-wasip1 target not found in Rust sysroot"
	fi
}
