...
# Rust version is dynamic, so we download the wasm32-wasip1 stdlib at
# build time based on the detected rustc version.
...
src_unpack() {
	...
	# Download and install wasm32-wasip1 stdlib into a local sysroot overlay.
	local rust_ver
	rust_ver="$(rustc --version | cut -d' ' -f2)" || die
	local rust_sysroot
	rust_sysroot="$(rustc --print sysroot)" || die
	local wasi_sysroot="${WORKDIR}/wasi-sysroot"

	einfo "Downloading wasm32-wasip1 stdlib for Rust ${rust_ver}..."
	wget -q "https://static.rust-lang.org/dist/rust-std-${rust_ver}-wasm32-wasip1.tar.xz" \
	...
	# Build a local sysroot: symlink the host target and libs from the
	# system sysroot, then add the downloaded wasm32-wasip1 target.
	mkdir -p "${wasi_sysroot}/lib/rustlib" || die
	ln -snf "${rust_sysroot}/lib/rustlib/x86_64-unknown-linux-gnu" \
		"${wasi_sysroot}/lib/rustlib/x86_64-unknown-linux-gnu" || die
	ln -snf "${rust_sysroot}/lib/rustlib/wasm32-unknown-unknown" \
		"${wasi_sysroot}/lib/rustlib/wasm32-unknown-unknown" || die
	cp -a "${T}/rust-std-${rust_ver}-wasm32-wasip1/rust-std-wasm32-wasip1/lib/rustlib/wasm32-wasip1" \
		"${wasi_sysroot}/lib/rustlib/" || die
	# Symlink shared libs and other rustlib contents needed by the compiler
	local f
	for f in "${rust_sysroot}/lib/rustlib"/*; do
		local base="${f##*/}"
		[[ -e "${wasi_sysroot}/lib/rustlib/${base}" ]] && continue
		ln -snf "${f}" "${wasi_sysroot}/lib/rustlib/${base}" || die
	done
	for f in "${rust_sysroot}/lib/"*.so; do
		[[ -e "${f}" ]] || continue
...
src_compile() {
	...
	# Run in a subshell to isolate host RUSTFLAGS from the wasm build.
	(
		...
		# Point rustc at our local sysroot overlay via the target-specific
		# env var. This merges with the project's .cargo/config.toml flags
		# (e.g. -C target-feature=+simd128) rather than overriding them.
		export CARGO_TARGET_WASM32_WASIP1_RUSTFLAGS="--sysroot ${wasi_sysroot}"
